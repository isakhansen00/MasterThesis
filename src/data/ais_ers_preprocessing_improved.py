...
- Voyage windows / AIS track selection now matches the OLD script logic,
  but with an optional earlier start point based on cumulative DCA catch:
    * OLD: starttime = Stopptidspunkt
    * NEW: try to move starttime back to the first DCA in the last 48h
           where cumulative rundvekt >= 80% of total rundvekt in that window.
           If this fails, fall back to Stopptidspunkt.
...
Other behaviour is unchanged:
    ...
    * Attach static info, gear usage, catches, etc.
"""
...
# -------- NEW: pick earlier start_ts based on cumulative DCA catch --------
def pick_start_ts_from_dca(
    dca_df: pd.DataFrame,
    callsign: str,
    ...
    """
    Try to move the voyage start earlier than default_start based on DCA rundvekt.

    Logic:
      - Look at all DCA rows for this callsign in the interval
           [default_start - window_hours, default_start]
      - Sort by stop_ts, compute cumulative rundvekt.
      - Let total = sum(rundvekt) in this window.
      - Find earliest row where cum_rundvekt >= threshold_ratio * total.
      - Use that row's stop_ts as new start_ts (but never later than default_start).

    If anything is missing (no DCA, no columns, total <= 0, etc.), return None.
    """
    if dca_df is None or dca_df.empty or pd.isna(default_start):
        return None

    call_col = pick_col(dca_df, ALIASES["callsign"])
    stop_col = pick_col(dca_df, ALIASES["stop_ts"])
    w_col    = pick_col(dca_df, ALIASES["rundvekt"])
    if not (call_col and stop_col and w_col):
        return None

    cs = str(callsign).strip().upper()

    t_max = pd.to_datetime(default_start)
    t_min = t_max - pd.Timedelta(hours=window_hours)

    sub = dca_df.loc[
        (dca_df[call_col].astype(str).str.strip().str.upper() == cs) &
        (dca_df[stop_col] >= t_min) &
        (dca_df[stop_col] <= t_max)
    ].copy()

    if sub.empty:
        return None

    sub = sub.sort_values(stop_col)
    w = pd.to_numeric(sub[w_col], errors="coerce").fillna(0.0)
    total = float(w.sum())
    if total <= 0.0:
        return None
    ...
    cum = w.cumsum()
    idx_first = cum[cum >= threshold].index.min()
    if pd.isna(idx_first):
        return None

    start_ts = sub.loc[idx_first, stop_col]
    if pd.isna(start_ts):
        return None

    start_ts = pd.to_datetime(start_ts)

    # Never move the start later than the default (we only want "earlier or equal")
    if start_ts > t_max:
        start_ts = t_max
...
def generate_voyages_like_old_script(
    ...
    ports_csv_path,
    limit=None,
    dca_for_start=None,   # NEW: DCA data for earlier start_ts logic
):
    """
    Reproduce the old script's AIS–ERS matching behaviour as closely as possible,
    with an optional earlier start_ts based on DCA cumulative catch.
    Returns:
    ...
    for _, row in tqdm(ers_df.iterrows(), desc="Collecting AIS tracks (legacy)", total=len(ers_df.index)):
        ...
        # NEW: try to move starttime earlier based on DCA cumulative catch
        if dca_for_start is not None:
            new_start = pick_start_ts_from_dca(
                dca_for_start,
                callsign=callsign,
        ...
        # AIS in window (exactly like old script, but with possibly earlier starttime)
        ...
        tracks.append({
            "mmsi": int(mmsi),
            "callsign": callsign,
            "start_time": starttime.to_pydatetime() if hasattr(starttime, "to_pydatetime") else starttime,
...
def match_and_insert(ais_files, ers_dir, year,
    ...
    # Use legacy-style voyage generation, but pass DCA for start_ts logic
